# WallHack Simulation — Cheat vs Anti-Cheat

An interactive information-security simulation showing how a wallhack uses client-visible
game data, how five client-side anti-cheat checks respond, and why server-side information
culling is the stronger architectural defense.

The project demonstrates, with measurements, a single claim in systems security:
...
2. **The naive architecture.** Set Server visibility to `All data — vulnerable`, then set
   Wallhack to `On — reveal hidden enemies`. A
...
3. **The architectural fix.** Leave the cheat running. Switch Server visibility to
   `Visible only — strongest`. The red
...
4. **The realistic architecture.** Switch Server visibility to `Buffered — balanced`.
   Yellow figures appear — NPCs the
...
5. **The defenses, one at a time.** Enable a defense, confirm it reports `[DETECTING]`, then
   set Wallhack to `Evasive — bypass anti-cheat` and tick its **bypass** box. The row turns red and reads
...
6. **The one that still wins.** Under **Advanced experiment tools**, enable Challenge-response
   with its bypass armed. The cheat
   answers every challenge correctly from its pristine source cache — content proves nothing.
   The simulation applies a fixed 40 ms bypass cost, so with **Analyze timing** enabled the
   server can flag the session on latency alone.

7. **The numbers.** Under **Advanced experiment tools**, run the benchmark (45 combinations),
   then read the conclusion panel that
...
```
index.html          page shell; loads dropdown styling and the trusted loader
styles.css
manifest.json       generated by tools/sign.mjs — do not edit by hand
src/
  dropdowns.js      accessible rounded dropdown presentation
